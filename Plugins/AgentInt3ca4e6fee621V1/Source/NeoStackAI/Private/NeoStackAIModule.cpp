...
static void ShowUpdateNotification()
{
	...
	if (bCoreUpdate)
	{
		HeaderText = FString::Printf(TEXT("v%s → v%s"), *CurrentVersion, *Info.LatestVersion);
		if (ExtCount > 0)
		{
			HeaderText += FString::Printf(TEXT(" + %d extension%s"),
				ExtCount, ExtCount == 1 ? TEXT("") : TEXT("s"));
		}
	}
	else if (ExtCount > 0)
	{
		HeaderText = FString::Printf(TEXT("%d extension update%s available"),
			ExtCount, ExtCount == 1 ? TEXT("") : TEXT("s"));
	}
	else
	{
		HeaderText = FString::Printf(TEXT("v%s"), *CurrentVersion);
	}
	if (!Info.Changelog.IsEmpty())
	{
		HeaderText += TEXT("\n\n");
		HeaderText += Info.Changelog;
	}
	if (ExtCount > 0)
	{
		// Names list — keep it short. UE's notification SubText doesn't
		// support tooltips natively, so inline the names with a separator.
		HeaderText += TEXT("\n\nIncludes: ");
		for (int32 i = 0; i < ExtCount; ++i)
		{
			if (i > 0) HeaderText += (i == ExtCount - 1) ? TEXT(" and ") : TEXT(", ");
			HeaderText += Info.PendingExtensionUpdates[i].DisplayName;
		}
	}
	NotifInfo.SubText = FText::FromString(HeaderText);
	...
	if (Info.bDownloadAvailable && !NeoStackKey.IsEmpty())
	{
		...
		const FString DownloadLabel = ExtCount > 0
			? FString::Printf(TEXT("Download v%s + %d extension%s"),
		...
		FNotificationButtonInfo DownloadBtn(
			FText::FromString(DownloadLabel),
		...
		const FString InstallLabel = ExtCount > 0
			? FString::Printf(TEXT("Install v%s + %d extension%s"),
				*Info.LatestVersion, ExtCount, ExtCount == 1 ? TEXT("") : TEXT("s"))
			: FString::Printf(TEXT("Install v%s"), *Info.LatestVersion);
		FNotificationButtonInfo InstallBtn(
			FText::FromString(InstallLabel),
	...
	else if (Info.bDownloadAvailable)
	{
		...
		NotifInfo.SubText = FText::FromString(FString::Printf(
			TEXT("v%s → v%s | Add a NeoStack API key in Settings > Chat & Agents > Chat Providers > NeoStack Cloud to enable auto-update"),
	...
	else if (ExtCount > 0 && !NeoStackKey.IsEmpty())
	{
		...
		const FString ExtLabel = FString::Printf(
			TEXT("Update %d extension%s"),
			ExtCount, ExtCount == 1 ? TEXT("") : TEXT("s"));
		FNotificationButtonInfo UpdateBtn(
			FText::FromString(ExtLabel),
			FText::FromString(TEXT("Install extension updates (requires editor restart)")),
			FSimpleDelegate::CreateStatic(&FNeoStackAIModule::InstallUpdateUnified),
			SNotificationItem::CS_None);
	...
	else
	{
		...
		NotifInfo.SubText = FText::FromString(FString::Printf(
			TEXT("v%s → v%s | No auto-update available for your platform yet"),
			*CurrentVersion, *Info.LatestVersion));
		NotifInfo.HyperlinkText = FText::FromString(TEXT("Open NeoStack.dev"));
		NotifInfo.Hyperlink = FSimpleDelegate::CreateLambda([]()
		{
	...
	FNotificationButtonInfo DismissBtn(
		FText::FromString(FString::Printf(TEXT("Stay on v%s"), *CurrentVersion)),
		FText::GetEmpty(),
