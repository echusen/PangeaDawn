...
public:
	...
	/**
	 * Unlocks a skill from the given skill tree on the server using the node's unique GUID.
	 * The function looks up the specified skill and applies its effects to the owning actor.
	 *
	 * @param SkillTreeTag The skill tree to use for the lookup.
	 * @param SkillNodeId The unique GUID identifying the skill node to unlock.
	 */
	...
	/**
	 * Checks if a skill can be unlocked based on requirements (level, skill points, parent nodes).
	 *
	 * @param SkillTreeTag The skill tree to use for the lookup.
	 * @param SkillNodeId The unique GUID identifying the skill node to check.
	 * @return True if the skill can be unlocked.
	 */
	UFUNCTION(BlueprintPure, Category = ACF)
	bool CanUnlockSkill(const FGameplayTag& SkillTreeTag, const FGuid& SkillNodeId) const;
	...
	/**
	 * Checks whether the specified skill node is currently active using its GUID.
	 ...
	 * clears internal tracking, and refunds all spent skill points.
	 */
	UFUNCTION(BlueprintCallable, Server, Reliable, Category = ACF)
	void Respec();
