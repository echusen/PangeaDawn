...
void UACFSkillTreeWidget::BuildFromNodes(const TArray<UAGSGraphNode*>& RootNodes)
{
    ...
    const float VerticalSpacing = BaseNodeDistance * 0.75f;
     // Posizione iniziale dell'albero

    // Create all widgets
    TSet<UACFBaseSkillNode*> Visited;
    
    // Per ogni albero radice (es: Warrior, Mage)
    for (int32 i = 0; i < RootNodes.Num(); i++)
    {
        UACFBaseSkillNode* RootNode = Cast<UACFBaseSkillNode>(RootNodes[i]);
        if (RootNode)
        {
            // Posiziona ogni albero radice in orizzontale con offset
            FVector2D RootPosition = TreeStartPosition + FVector2D(i * BaseNodeDistance * 3, 0);
