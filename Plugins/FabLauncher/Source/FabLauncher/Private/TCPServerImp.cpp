...
FTCPServer::FTCPServer() 
{
	...
	FString ThreadName(FString::Printf(TEXT("FabLauncher%i"), WorkerCounter.Increment()));
			...
			if (RecievedJson != TEXT(""))
			{
				...
				{			
					...
					AsyncTask(ENamedThreads::GameThread, [this, RecievedJson]() {
						// Todo: here, do we need to wait for the previous completion ?
						if (IsGarbageCollecting() || GIsSavingPackage) return;
						else FAssetsImportController::Get()->ImportAssets(RecievedJson);	
					});
					RecievedJson.Empty();
				}
			}
