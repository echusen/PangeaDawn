...
#include "FabInterchangeInstancedFoliageTypeFactory.h"
...
UClass* UFabInterchangeInstancedFoliageTypeFactory::GetFactoryClass() const
{
	return UFoliageType_InstancedStaticMesh::StaticClass();
}

UInterchangeFactoryBase::FImportAssetResult UFabInterchangeInstancedFoliageTypeFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UFabInterchangeInstancedFoliageTypeFactory::BeginImportAsset_GameThread);
	...
	auto CouldNotCreateDemoObjectLog = [this, &Arguments, &ImportAssetResult](const FString& Info)
	{
		...
		Message->Text                            = FText::FromString(
			FString::Printf(TEXT("UFabInterchangeInstancedFoliageTypeFactory: Could not create UFoliageType_InstancedStaticMesh asset %s. Reason: %s"), *Arguments.AssetName, *Info)
		);
		ImportAssetResult.bIsFactorySkipAsset = true;
	};

	const UFabInterchangeInstancedFoliageTypeFactoryNode* DemoObjectFactoryNode = Cast<UFabInterchangeInstancedFoliageTypeFactoryNode>(Arguments.AssetNode);
...
void UFabInterchangeInstancedFoliageTypeFactory::SetupObject_GameThread(const FSetupObjectParams& Arguments)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UFabInterchangeInstancedFoliageTypeFactory::SetupObject_GameThread);
	...
	#if WITH_EDITOR
	{
		...
		if (const UFabInterchangeInstancedFoliageTypeFactoryNode* InstancedFoliageTypeFactoryNode = Cast<UFabInterchangeInstancedFoliageTypeFactoryNode>(FactoryNode))
		{
