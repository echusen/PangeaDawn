...
UWorld* FSMPreviewUtils::DuplicateWorldForSimulation(const FString& PackageName, UWorld* OwningWorld, FWorldContext& InWorldContext)
{
	...
	SimulationLevelPackage->SetLoadedPath(EditorLevelPackage->GetLoadedPath());
	SimulationLevelPackage->SetSavedHash(EditorLevelPackage->GetSavedHash());
	...
	const UGameInstance* GameInstance = EditorLevelWorld->GetGameInstance();
	USMPreviewGameInstance* ClonedGameInstance = NewObject<USMPreviewGameInstance>(GameInstance ? GameInstance->GetOuter() : GEngine /* Should be GEngine */,
	...
	// We need a valid world context before initializing.
	InWorldContext.SetCurrentWorld(SimulationLevelWorld);
